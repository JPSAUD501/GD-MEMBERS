...
async function inviteChecker(invite, client, guildid, datafile){
  ...
  if(updateInvite) return deleteInvite(client, updateInvite, `Atenção <@!${invite.inviter.id}>! O seu convite foi atualizado.\nParece que você atualizou um de seus convites!\n\nCaso você tenha tentado criar um novo, devo avisa-lo que apénas o ultimo convite gerado é valido pois só é permitido um link pré-aprovado ativo por membro! (Não se preocupe a atualização de links não gasta pontos do seu score!)`);
