...
async function newPrivateGuildMember (client, guildId, member) {
  ...
  let joinedMember = {}
  try { joinedMember = await guild.members.fetch(member.user.id).catch() } catch (e) {}
  if (!joinedMember) {
    ...
    const mainChatChannel = await member.guild.channels.cache.find(channel => channel.name === '🌎┆geral-privado')
