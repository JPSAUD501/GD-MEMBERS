...
function buttonClicked(client, interaction, datafile, guildid){
  ...
  guild.members.fetch(interaction.customId).then((user) => {
    ...
    client.channels.cache.get(process.env['logchannel']).send({content:`**AUTORIZAÇÃO** - O membro **"${user.user.username}" - "<@${user.id}>"** foi autorizado a ser membro do servidor por **"${interaction.user.username}" - "<@${interaction.user.id}>"** usando **1** ponto dos seus **${data.memberList[interaction.user.id].points + 1}**.`});
